...
export const defineCommands = (replServer, parseOpts) => {
...
const defineGroup = ({ replServer, parseOpts, action, commands }) => {
  commands.forEach(({ attr, name, help }) => {
    defineCommand({ replServer, parseOpts, attr, name, help, action })
  })
}

const defineCommand = ({ replServer, parseOpts, attr, name, help, action }) => {
